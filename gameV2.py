...
def main():
    ...
    pygame.init()    #要初始化后pygame才能使用
    FPSCLOCK = pygame.time.Clock()  #定时器，控制每个循环多久一次
    SCREEN = pygame.display.set_mode((screen_width, screen_height))
    pygame.display.set_caption('fudan gesture bird')
...
def mainGame(movementInfo):
    ...
    frame_rate = cap.get(5)  # 获取帧速率
    print("frame rate:", frame_rate)
    count=0
    while True:
        ret, frame = cap.read()  # 帧读取是否正确，每一帧的图像--三维矩阵
        ...
        if crashTest[0]:   #如果掉到了地上或者撞击管道，game over
            return {
                'y': playery,
...
def prediction(pred):
    if pred==0 or pred==4 or pred==12:
        #print('operation')
        sign=3
        #elif pred==1:  print('stop')
        #elif pred==3:  print('simple')
    elif pred==5:
        #print('start')
        sign=1
...
# 读取视频
    ...
    # 构造高斯混合模型
    model_image = cv2.createBackgroundSubtractorMOG2()
    """
